...
export const profileAPI = {
  ...
  async saveProfilePhoto(photoFile: File) {
    ...
    return API.post(baseUrl(`/profile/upload`), await expandHeadersForPhoto(), formData);
  },
  async changeBackground(url: string) {
    return API.put(baseUrl("/profile/changeBackground"), await expandHeadersForPhoto(), { url });
  },
  async updateProfile(user_id: string, payload: string) {
    return API.put(baseUrl(`/users/${user_id}`), await expandHeaders(), payload);
  },
};
