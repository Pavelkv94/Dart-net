...
export const HeadDropdownMenu = ({ setOpenProfileMenu, exit, t }: { setOpenProfileMenu: (value: boolean) => void; exit: () => void; t: TFunction }) => {
  const onStopPropagation = (e: MouseEvent<HTMLElement>) => e.stopPropagation();
  const onClose = () => setOpenProfileMenu(false);

  const email = useSelector<AppStateType, string | undefined>((state) => state.app.user?.email);
